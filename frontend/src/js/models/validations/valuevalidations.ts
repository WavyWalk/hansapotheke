...
export const validateModelCollectionNotEmpty = (model: BaseModel | any, propertyName: string, message?: string) => {
    ...
    if (!value || (value.array?.length ?? 0) < 1) {
        model.addErrorFor(propertyName, message ?? 'At least one must be assigned')
        return false
    }
    return true
}
...
export const validateImageHref = <T extends BaseModel>(model: T, propertyName: keyof T) => {
    const property = propertyName as string
    const value = model[propertyName] as unknown as any
    model.removeErrorsFor(property)
    if (!value) {
        model.addErrorFor(property, 'Please provide link to image')
        return false
    }
    if (value.match(/\.(jpeg|jpg|png|webp)$/) === null) {
        model.addErrorFor(property, 'Please provide a correct link to image')
    }
    return true
}